...
- [ ] **Nutrition for TheMealDB and TheCocktailDB** - recipe and cocktail ingredients are currently stored as unresolved `FoodPortion` stubs. The next step is to map them to real `FoodProduct` entries via fuzzy matching against the food databases, enabling nutritional computation for these recipes and cocktails. This includes a confidence threshold, manual override UX, and Room persistence of confirmed mappings. Unaffected by FatSecret below - a different data source with its own recipes doesn't resolve TheMealDB/TheCocktailDB's own ingredient stubs.
- [x] **FatSecret as a recipe + food data source** - a new source with its own recipe and food databases, providing real structured nutrition natively (no fuzzy matching needed for FatSecret's own items). First source producing both `ProductType.FOOD` and `ProductType.RECIPE` from one account, and the first source in this project without a public API key fallback (see Data Sources).
